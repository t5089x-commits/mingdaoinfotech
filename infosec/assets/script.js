...
(function () {
  ...
  /* ---------- 互動式 Merkle Tree（單元 4）：邊打字邊往上算雜湊 ---------- */
  var _merkleVal = { A: "", B: "", C: "", D: "" };
  var _merkleCache = { A: "", B: "", C: "", D: "", AB: "", CD: "", ROOT: "" };
  function _mkShort(h) { return h ? (h.slice(0, 10) + "…" + h.slice(-8)) : "—"; }
  function _mkRender(node, hashVal) {
    var el = document.getElementById("mk-hash-" + node);
    if (!el) return;
    var changed = _merkleCache[node] !== hashVal;
    _merkleCache[node] = hashVal;
    var label = (node.length === 1) ? ("Hash(" + node + ")：") : (node === "ROOT" ? "Merkle Root：" : "雜湊值：");
    el.textContent = hashVal ? (label + _mkShort(hashVal))
      : (node.length === 1 ? "Hash(" + node + ")：尚未輸入" : "尚未就緒（下面欄位還沒填齊）");
    el.title = hashVal || "";
    if (changed) {
      var box = el.closest(".mk-node");
      if (box) { box.classList.remove("mk-flash"); void box.offsetWidth; box.classList.add("mk-flash"); }
    }
  }
  window.merkleUpdate = function (leaf) {
    var input = document.getElementById("mk-in-" + leaf);
    if (!input) return;
    _merkleVal[leaf] = input.value;
    var h = {};
    ["A", "B", "C", "D"].forEach(function (k) {
      h[k] = _merkleVal[k] ? _sha256Hex(_merkleVal[k]) : "";
      _mkRender(k, h[k]);
    });
    var ab = (h.A && h.B) ? _sha256Hex(h.A + h.B) : "";
    var cd = (h.C && h.D) ? _sha256Hex(h.C + h.D) : "";
    _mkRender("AB", ab);
    _mkRender("CD", cd);
    var root = (ab && cd) ? _sha256Hex(ab + cd) : "";
    _mkRender("ROOT", root);
  };
  function _mkDrawLines() {
    var wrap = document.getElementById("merkleWidget");
    var svg = document.getElementById("mkLines");
    if (!wrap || !svg) return;
    var wr = wrap.getBoundingClientRect();
    if (!wr.width || !wr.height) return;
    svg.setAttribute("viewBox", "0 0 " + wr.width + " " + wr.height);
    var pairs = [["A", "AB"], ["B", "AB"], ["C", "CD"], ["D", "CD"], ["AB", "ROOT"], ["CD", "ROOT"]];
    var out = "";
    pairs.forEach(function (p) {
      var a = wrap.querySelector('.mk-node[data-node="' + p[0] + '"]');
      var b = wrap.querySelector('.mk-node[data-node="' + p[1] + '"]');
      if (!a || !b) return;
      var ar = a.getBoundingClientRect(), br = b.getBoundingClientRect();
      var x1 = ar.left + ar.width / 2 - wr.left, y1 = ar.top - wr.top;
      var x2 = br.left + br.width / 2 - wr.left, y2 = br.top + br.height - wr.top;
      out += '<line x1="' + x1 + '" y1="' + y1 + '" x2="' + x2 + '" y2="' + y2 + '"/>';
    });
    svg.innerHTML = out;
  }
  window.addEventListener("resize", function () {
    clearTimeout(window._mkResizeT);
    window._mkResizeT = setTimeout(_mkDrawLines, 150);
  });
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    if (document.getElementById("merkleWidget")) _mkDrawLines();
