...
密鑰只有一把、五科通用（跟每一科單元鎖定的 `teacherKey` 是同一個值，
目前是 `t5089x`）：課程首頁的「🔒 教師專區」→ `video-access.html` 輸入
...
的資安教材，不是漏洞。要換密鑰的話，`teacher-key.js` 跟五科各自的
`assets/progress.js` 要一起改，兩邊要維持一致。
