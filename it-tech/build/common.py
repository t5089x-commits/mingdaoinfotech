...
def _compiler_cta():
    return f'''<a class="compiler-cta cta-fixed" href="{PROGRAMIZ}" target="_blank" rel="noopener" title="開啟 Programiz 線上 Python 編譯器：把上面的程式碼複製過去就能執行（另開新分頁）">
  <span class="ic">🐍</span>
...
<main class="wrap" style="max-width:820px">
  ...
  <div class="callout" style="border-left:4px solid var(--warn)">
    ...
    <p>老實說：<strong>就是「前端密碼」</strong>。密鑰 <code>……</code> 明碼放在 <code>teacher-key.js</code>，比對在你的瀏覽器裡跑，而影片頁又是一個直接打網址就能開的獨立網頁。所以它比較像是「在門上貼一張紙寫『非教師請勿進入』」——擋得住願意守規矩的人，但擋不住真的想進去、又懂一點技術的人。</p>
