...
<div class="callout"><span class="t">🌏 台灣 vs. 大陸用語對照</span>
<p style="margin-bottom:0">很多電腦零組件與資訊名詞，台灣和大陸的講法不一樣。上網找資料、看影片或逛購物網站時，常會遇到大陸的說法；先認識這些對應名稱，搜尋時才不會卡住。</p></div>
<div class="table-wrap"><table class="center dual-term">
<thead><tr><th>台灣用語</th><th>大陸用語</th><th>英文</th><th>台灣用語</th><th>大陸用語</th><th>英文</th></tr></thead>
<tbody>
<tr><td>CPU／處理器</td><td>CPU／處理器／芯片</td><td>CPU</td><td>電源供應器</td><td>電源</td><td>PSU</td></tr>
<tr><td>記憶體</td><td>內存／內存條</td><td>RAM</td><td>音效卡</td><td>聲卡</td><td>Sound Card</td></tr>
<tr><td>主機板</td><td>主板</td><td>Motherboard</td><td>網路卡</td><td>網卡</td><td>Network Card</td></tr>
<tr><td>顯示卡</td><td>顯卡</td><td>GPU / Graphics Card</td><td>散熱器</td><td>散熱器</td><td>Cooler</td></tr>
<tr><td>硬碟</td><td>硬盤</td><td>HDD</td><td>機殼</td><td>機箱</td><td>Case</td></tr>
<tr><td>固態硬碟</td><td>固態硬盤</td><td>SSD</td><td></td><td></td><td></td></tr>
</tbody></table></div>
<div class="reveal" onclick="toggleWork(this)"><span class="wk-arrow">▶</span> 📚 更多資訊常用術語對照（點開）</div>
<div class="answer">
<p style="margin-top:0">除了硬體零件，寫程式、上網、用電腦時也常遇到兩岸不同的說法，一併整理如下：</p>
<div class="table-wrap"><table class="center dual-term">
<thead><tr><th>🇹🇼 台灣</th><th>🇨🇳 大陸</th><th>英文</th><th>🇹🇼 台灣</th><th>🇨🇳 大陸</th><th>英文</th></tr></thead>
<tbody>
<tr><td>軟體</td><td>軟件</td><td>Software</td><td>資料夾</td><td>文件夾</td><td>Folder</td></tr>
<tr><td>硬體</td><td>硬件</td><td>Hardware</td><td>伺服器</td><td>服務器</td><td>Server</td></tr>
<tr><td>程式</td><td>程序</td><td>Program</td><td>網路</td><td>網絡</td><td>Network</td></tr>
<tr><td>程式碼</td><td>代碼</td><td>Code</td><td>網際網路</td><td>互聯網</td><td>Internet</td></tr>
<tr><td>演算法</td><td>算法</td><td>Algorithm</td><td>網頁</td><td>網頁</td><td>Web Page</td></tr>
<tr><td>函式</td><td>函數</td><td>Function</td><td>網址</td><td>網址</td><td>URL</td></tr>
<tr><td>變數</td><td>變量</td><td>Variable</td><td>介面</td><td>界面</td><td>Interface</td></tr>
<tr><td>陣列</td><td>數組</td><td>Array</td><td>快取</td><td>緩存</td><td>Cache</td></tr>
<tr><td>物件</td><td>對象</td><td>Object</td><td>雲端運算</td><td>雲計算</td><td>Cloud Computing</td></tr>
<tr><td>執行緒</td><td>線程</td><td>Thread</td><td>使用者</td><td>用戶</td><td>User</td></tr>
<tr><td>行程</td><td>進程</td><td>Process</td><td>帳號</td><td>賬號</td><td>Account</td></tr>
<tr><td>資料庫</td><td>數據庫</td><td>Database</td><td>影片</td><td>視頻</td><td>Video</td></tr>
<tr><td>資料</td><td>數據</td><td>Data</td><td>音訊</td><td>音頻</td><td>Audio</td></tr>
<tr><td>檔案</td><td>文件</td><td>File</td><td></td><td></td><td></td></tr>
</tbody></table></div>
</div>
...
<div class="reveal" onclick="toggleWork(this)"><span class="wk-arrow">▶</span> 🔎 進階：看懂主機板「型號」＋認識 CPU「腳位」（點開）</div>
<div class="answer">
<p style="margin-top:0"><strong>主機板型號怎麼看？</strong>通常是「<strong>品牌＋系列＋晶片組＋板型</strong>」拼起來。以 <code>華碩 TUF GAMING B760M-PLUS</code> 為例：</p>
<ul class="tidy">
  <li><strong>華碩</strong>＝品牌、<strong>TUF GAMING</strong>＝產品系列。</li>
  <li><strong>B760</strong>＝晶片組（開頭字母代表等級：Intel 由低到高 <code>H → B → Z</code>；AMD 為 <code>A → B → X</code>），數字越大通常越新。</li>
  <li>型號裡的 <strong>M</strong>＝板型是 <strong>mATX 小板</strong>（沒有 M 多半是 ATX 大板、<strong>I</strong> 則是 ITX 迷你板）。</li>
</ul>
<p><strong>什麼是「腳位（插槽 / Socket）」？</strong>就是 CPU 插進主機板的那個座，上面有成千上百個接點。<strong>CPU 的腳位一定要和主機板一樣</strong>才裝得上——買 CPU 和主機板必須「配對」，不能亂配。近年常見腳位：</p>
<div class="table-wrap"><table class="center">
<thead><tr><th>品牌</th><th>近年主要腳位</th><th>搭配的 CPU（例）</th></tr></thead>
<tbody>
<tr><td>Intel</td><td><strong>LGA 1851</strong>（新）、LGA 1700</td><td>Core Ultra 200 系列 / 12～14 代</td></tr>
<tr><td>AMD</td><td><strong>AM5</strong>（新）、AM4</td><td>Ryzen 7000／9000 / Ryzen 5000</td></tr>
</tbody></table></div>
<p style="margin-bottom:0">所以挑機的順序常是：<strong>先決定 CPU → 看它的腳位 → 再挑「同腳位、晶片組等級合適」的主機板</strong>。（整理自歐飛先生「主機板的選購與推薦」）</p></div>
''') + sec(2, "CPU 中央處理器", '''
<div class="part">
...
<div class="callout"><span class="t">🔧 補充：現在還需要「超頻」嗎？</span>
<p>上表的 <strong>K</strong>（Intel）、<strong>X</strong>（AMD）代表「不鎖頻、可超頻」。<strong>超頻</strong>是指把 CPU 的運作頻率手動調得比原廠更高、以換取更多效能；以前很多電腦玩家很愛這樣做，還會特地挑這類型號。</p>
<p style="margin-bottom:0">不過在<strong>現在的 CPU 上，超頻已經「吃力又效果有限」</strong>：新款 CPU 出廠就會自己偵測溫度與功耗、動態把頻率衝到接近極限（Intel 的 <strong>Turbo Boost</strong>、AMD 的 <strong>Precision Boost</strong>），手動超頻能再多擠出的效能通常只有<strong>幾個百分比</strong>，卻要付出更多發熱、更吵的散熱、更耗電，還得多花錢買高階主機板和強力散熱器，穩定性也可能變差。所以對大多數人（文書、上網、甚至一般遊戲）來說，<strong>維持原廠設定就很夠用</strong>，不必為了超頻硬去追 K／X 型號。（參考：XDA、ASUS Edge Up 等 2025 年評析）</p></div>
''') + sec(3, "RAM 主記憶體與顯示卡", '''
<div class="grid cols-2">
...
<div class="reveal" onclick="toggleWork(this)"><span class="wk-arrow">▶</span> 🔎 進階：記憶體的「時脈、CL、雙通道」怎麼看（點開）</div>
<div class="answer">
<ul class="tidy" style="margin-top:0">
  <li><strong>世代：</strong>目前是 <strong>DDR5</strong>（新）與 <strong>DDR4</strong>（舊），兩者<strong>不相容</strong>，要看主機板支援哪一種。</li>
  <li><strong>時脈（頻率）：</strong><code>DDR5-6000</code>、<code>DDR4-3200</code> 後面那個數字，代表<strong>每秒能傳幾次資料</strong>，數字越大越快（單位 MT/s，口語常說成 MHz）。</li>
  <li><strong>CL（CAS Latency，延遲）：</strong><code>CL30</code>、<code>CL16</code> 代表「收到指令到吐出資料」要等幾個週期，<strong>數字越小反應越快</strong>。</li>
  <li><strong>時脈和 CL 要一起看：</strong>時脈高的記憶體 CL 通常也比較大，實際快慢是兩者綜合。<strong>一般人不用太糾結 CL</strong>——先選對 DDR 世代、挑主機板／CPU 支援的時脈就好。</li>
  <li><strong>雙通道：</strong>插<strong>兩條一模一樣</strong>的記憶體（例如 16G×2），頻寬加倍，比單一條 32G 更順，<strong>對內顯和遊戲特別有感</strong>。所以買 32G 常建議「16G×2」而不是「單條 32G」。</li>
</ul>
<p style="margin-bottom:0;font-size:.85rem;color:var(--text-faint)">整理自歐飛先生「RAM 記憶體的選購與推薦」。</p></div>
<div class="reveal" onclick="toggleWork(this)"><span class="wk-arrow">▶</span> 🤖 想在自己電腦「跑本地 AI」要多大記憶體？（點開）</div>
<div class="answer">
<p style="margin-top:0">把 AI 大型語言模型（像 ChatGPT 那種，簡稱 <strong>LLM</strong>）下載到自己電腦離線跑，<strong>非常吃記憶體</strong>——模型的「參數量（B＝十億）」越大越吃。粗略門檻（會因「量化壓縮」而變動）：</p>
<div class="table-wrap"><table class="center">
<thead><tr><th>模型大小</th><th>大約需要的記憶體</th><th>感覺</th></tr></thead>
<tbody>
<tr><td>約 7B（70 億參數）</td><td><strong>8～16 GB</strong></td><td>入門，一般筆電勉強可跑</td></tr>
<tr><td>約 13B</td><td><strong>16～32 GB</strong></td><td>要中高階機器</td></tr>
<tr><td>30B 以上</td><td><strong>32 GB 以上</strong></td><td>要高階、記憶體要很大</td></tr>
</tbody></table></div>
<p style="margin-bottom:0">如果改<strong>用顯示卡</strong>跑（快很多），吃的就是<strong>顯示卡的記憶體（VRAM）</strong>。所以想玩本地 AI，<strong>系統 RAM 和顯卡 VRAM 都要大</strong>——這也是近年大家買電腦越來越在意記憶體容量的原因之一。</p></div>
...
<p style="margin-top:14px"><strong>為什麼硬碟速度會影響開機？</strong>因為<strong>作業系統（Windows 等）的所有檔案本身就存放在硬碟裡</strong>。每次開機，電腦要先把作業系統的資料<strong>從硬碟讀出來、載入到記憶體（RAM）</strong>，CPU 才能執行它、讓畫面進入桌面；開啟每個程式也一樣——程式檔存在硬碟，要先被讀進記憶體才能跑。所以<strong>硬碟讀取越快，開機和開程式就越快</strong>，這也是換上 NVMe SSD 後感受最明顯的原因。</p>
<div class="bootflow" role="img" aria-label="開機資料流動示意圖：按下電源開機後，從硬碟讀取作業系統資料並載入到記憶體 RAM，CPU 執行後螢幕進入桌面">
  <div class="bf-step"><div class="bf-ic">🔌</div><div class="bf-t">① 按下電源<br>開機</div></div>
  <div class="bf-arr">→</div>
  <div class="bf-step"><div class="bf-ic">💽→🧠</div><div class="bf-t">② 從<strong>硬碟</strong>讀取作業系統資料，載入到<strong>記憶體（RAM）</strong></div></div>
  <div class="bf-arr">→</div>
  <div class="bf-step"><div class="bf-ic">🖥️</div><div class="bf-t">③ CPU 執行作業系統<br>螢幕<strong>進入桌面</strong></div></div>
</div>
<p style="font-size:.8rem;color:var(--text-faint);margin:2px 0 0;text-align:center">硬碟＝長期存放資料的地方（斷電也還在）；記憶體（RAM）＝執行時暫時放資料的地方（速度快、斷電就清空）。</p>
''') + sec(5, "電源、機殼與螢幕", '''
<div class="grid cols-3">
...
<div class="reveal" onclick="toggleWork(this)"><span class="wk-arrow">▶</span> 🧰 進階：把零件裝進機殼時的小提醒（點開）</div>
<div class="answer">
<p style="margin-top:0">把零件裝進機殼時，有幾個新手常忽略的重點：</p>
<ul class="tidy">
  <li><strong>先鎖銅柱（螺柱）：</strong>機殼底板要先鎖上對應主機板孔位的<strong>銅柱</strong>，主機板才不會直接碰到金屬<strong>短路</strong>。</li>
  <li><strong>先裝「I/O 擋板」：</strong>主機板後端的金屬擋板要先卡進機殼背面，再放主機板進去。</li>
  <li><strong>CPU 對三角記號：</strong>CPU 有一個小三角形，要對準插槽的三角形方向，<strong>輕放</strong>不要硬壓，以免壓歪針腳。</li>
  <li><strong>記憶體壓到「喀」一聲：</strong>對準缺口、兩邊平均施力壓到卡扣扣上；裝兩條要插「<strong>隔一槽的同色插槽</strong>」才是雙通道。</li>
  <li><strong>顯示卡要鎖螺絲：</strong>插進 PCIe x16 後，記得鎖上機殼背檔的螺絲固定，別讓顯卡下垂。</li>
  <li><strong>電源線要接齊：</strong>主機板 24-pin、CPU 8-pin、顯卡 8-pin(6+2) 都要接上，<strong>少接一個就開不了機</strong>。</li>
  <li><strong>理線與風扇方向：</strong>線盡量走背板整理，風扇注意「<strong>前面吸、後面／上面吹</strong>」的風道。</li>
  <li><strong>開機前再檢查一次：</strong>確認沒有螺絲掉在板子上、線都接好，再通電。</li>
</ul>
<p style="margin-bottom:0;font-size:.85rem;color:var(--text-faint)">（可搭配本頁最下方「需求分析與組裝」的相容性提醒一起看。）</p></div>
...
<div class="callout"><span class="t">📚 延伸閱讀：想更深入了解怎麼配電腦</span>
<p style="margin-bottom:0">本單元不少「怎麼挑、怎麼配」的觀念，參考了台灣知名的電腦組裝部落格 <strong>歐飛先生</strong>（他有很多新手向的選購與組裝問答）。想深入的同學可以去看看：<a href="https://ofeyhong.pixnet.net/blog" target="_blank" rel="noopener">歐飛先生 痞客邦部落格 ↗</a>（主機板、RAM、顯卡、每月組裝菜單等都有整理）。</p></div>
''') + exercise("課堂練習", '''
<ol>
...
<div class="reveal" onclick="toggleWork(this)"><span class="wk-arrow">▶</span> 🎬 延伸：如果改成要「剪輯影音」，先加強顯示卡還是記憶體？（點開）</div>
<div class="answer">
<p style="margin-top:0">剪影片和「文書上網」正好相反——它<strong>三個都吃</strong>：CPU、記憶體、顯示卡。真要排優先順序：</p>
<ul class="tidy">
  <li><strong>① 先把記憶體補到 32GB：</strong>剪輯（尤其 4K、開多軌、上特效）很吃 RAM，不夠會<strong>卡頓、預覽不順</strong>。這通常是第一個瓶頸，所以「記憶體」先顧。</li>
  <li><strong>② CPU 要多核心、SSD 要大又快：</strong>輸出（算圖）主要靠 CPU 核心數；素材放在大容量 <strong>NVMe SSD</strong> 讀取才快。</li>
  <li><strong>③ 顯示卡負責「加速」：</strong>剪輯軟體（Premiere、DaVinci）會用顯卡做<strong>即時預覽、特效與輸出加速</strong>，中階獨顯就很有感，預算夠再往上升，對輸出速度幫助明顯。</li>
</ul>
<p style="margin-bottom:0"><strong>一句話：</strong>先讓「記憶體（32GB）＋ 多核 CPU ＋ 大容量 NVMe」到位、避免卡頓，<strong>再</strong>用一張中階以上的獨立顯卡加速預覽與輸出。（觀念整理自歐飛先生等組裝建議）</p></div>
</li>
</ol>''')
...
<section class="block">
  ...
  <div class="reveal" onclick="toggleWork(this)"><span class="wk-arrow">▶</span> 點開老師那台主機規格</div>
  <div class="answer">
